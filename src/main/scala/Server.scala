...
object Server {
  ...
  var data: scala.collection.concurrent.Map[String, Any] =
  ...
  class RedisThread(socket: Socket) extends Thread {
    ...
    def exec(commands: List[Any]): Any =
      commands.head.asInstanceOf[String].toUpperCase match {
        ...
        case "INCRBY" =>
          store.incrBy(
            commands(1).asInstanceOf[String],
            commands(2).toString.toLong
          )
        case "DECRBY" =>
          store.decrBy(
            commands(1).asInstanceOf[String],
        ...
        case "EXISTS" =>
          store.exists(commands.drop(1).asInstanceOf[List[String]])
        case "COMMAND" => "OK"
    ...
    def getInputWithLength(
      ...
      if (commands.length == length) {
        ...
        head.charAt(0) match {
          case '$' =>
            getInputWithLength(input, commands :+ input.next(), length)
          case ':' =>
            getInputWithLength(
              input,
              commands :+ toLongIfAble(head.diff(":")),
              length
            )
    ...
    def onSet(commands: List[Any]): Any = commands match {
      ...
      case List(key: String, value, option: String) =>
        setWithOption(key, value.toString, option)
      case _ => List("-", "invalid argument" + commands)
    }

    def setWithOption(key: String, value: String, option: String): Any =
